...
class TestCompileOptions(TestCase):
    def test_register_and_lookup(self):
        with fake_backend_routes():
            route = (fake_config_module, "e_bool")
            self.assertEqual(get_compile_option_owner("fake_backend_bool"), route)
            # dashed spelling resolves to the same route
            self.assertEqual(get_compile_option_owner("fake-backend-bool"), route)
        self.assertIsNone(get_compile_option_owner("fake_backend_bool"))
    ...
    def test_register_option_conflicts(self):
        with fake_backend_routes():
            ...
            self.assertEqual(
                get_compile_option_owner("fake_backend_bool"),
                (fake_config_module, "e_string"),
            )
    ...
    def test_register_rejects_shared_owner_conflict(self):
        with fake_backend_routes():
            with self.assertRaisesRegex(RuntimeError, "already claimed"):
                register_backend_for_device(
                    "fake_device_b",
    ...
    def test_patch_inductor_backend_preserves_options(self):
        with fake_backend_routes():
            route = (fake_config_module, "e_bool")
            with self.assertRaisesRegex(RuntimeError, "abort patch"):
                with patch_inductor_backend("fake_device_a"):
                    self.assertEqual(
                        get_compile_option_owner("fake_backend_bool"), route
                    )
                    with patch_compile_options({"fake_backend_bool": False}):
                        self.assertFalse(fake_config_module.e_bool)
                    raise RuntimeError("abort patch")
            self.assertEqual(get_compile_option_owner("fake_backend_bool"), route)
    ...
    def test_apply_options_unknown_name_rejected(self):
        with fake_backend_routes():
            # the known-option list includes backend-declared names
            with self.assertRaisesRegex(
                RuntimeError, "Unexpected optimization option.*fake_backend_bool"
            ):
                torch._TorchCompileInductorWrapper(
                    None, {"unregistered_option": True}, None
                )

    def test_list_options_includes_routed(self):
        with fake_backend_routes():
            names = torch._inductor.list_options()
            self.assertIn("fake_backend_bool", names)
            self.assertIn("max_fusion_size", names)
    ...
    def test_deferred_backend_apply_options(self):
        ...
        with deferred_backend_routes() as registered:
            wrapper = torch._TorchCompileInductorWrapper(
                None, {"fake_backend_bool": False}, None
            )
            self.assertEqual(wrapper.config["fake_backend_bool"], False)
    ...
    def test_patch_compile_options_decorator_reentry(self):
        ...
        self.assertTrue(fake_config_module.e_bool)
        self.assertEqual(codegen_common.snapshot_routed_options(), {})

    def test_patch_compile_options_nested(self):
        with fake_backend_routes():
            with patch_compile_options({"fake_backend_bool": False}):
                self.assertFalse(fake_config_module.e_bool)
                with patch_compile_options({"fake_backend_bool": True}):
                    self.assertTrue(fake_config_module.e_bool)
                    self.assertEqual(
                        codegen_common.snapshot_routed_options(),
                        {FAKE_MODULE: {"e_bool": True}},
                    )
                self.assertFalse(fake_config_module.e_bool)
                self.assertEqual(
                    codegen_common.snapshot_routed_options(),
                    {FAKE_MODULE: {"e_bool": False}},
                )
            self.assertTrue(fake_config_module.e_bool)
            self.assertEqual(codegen_common.snapshot_routed_options(), {})
    ...
    def test_get_patched_config_dict_forced_value(self):
        with fake_backend_routes():
            with mock.patch.object(
                ...
                result = get_patched_config_dict({"fake_backend_bool": False})
                self.assertTrue(result["fake_backend_bool"])

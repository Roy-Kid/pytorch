...
class TestAutocast(TestCase):
    ...
    def test_compile_with_backend_autocast_internal_defaults(self):
        class InternalDefaultsAutocast(torch.amp.autocast_mode.autocast):
            def __init__(self):
                super().__init__("openreg", dtype=torch.bfloat16, enabled=False)

        with mock.patch.object(torch.openreg.amp, "autocast", InternalDefaultsAutocast):
            backend = EagerAndRecordGraphs()

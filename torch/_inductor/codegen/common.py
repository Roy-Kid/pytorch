...
def list_compile_options() -> list[str]:
    """Names of the compile options declared by registered backends."""
    init_backend_registration()
    names: list[str] = []
    for codegen in device_codegens.values():
        names.extend(codegen.compile_options)
    return names
...
def patch_compile_options(config_patches: dict[str, Any] | None):
    ...
    for name, value in (config_patches or {}).items():
        if name.replace("-", "_") not in config._config:  # type: ignore[attr-defined]
